...
int print_data2(FILE *fp, char *filename, struct tline_parameters *data, int which_Z)
{
  ...
  {
    if(data->Er < 0.0)
      fprintf(fp,"%s conductors= %d Er= MIXED C= %.4f pF/m L= %.4f nH/m Zo= %.4f Ohms v= %4g m/sv_f= %.4f VERSION=%s\n",filename,data->conductors, data->C,data->L,data->Zo, data->v, data->v_f, VERSION);
    else 
      fprintf(fp,"%s conductors= %d Er= %.4f C= %.4f pF/m L= %.4f nH/m Zo= %.4f Ohms v= %4g m/sv_f= %.4f VERSION=%s\n",filename,data->conductors, data->Er, data->C,data->L,data->Zo, data->v, data->v_f,VERSION);
  }
