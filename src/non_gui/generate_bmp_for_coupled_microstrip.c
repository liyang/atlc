...
int main(int argc, char **argv) /* Read parameters from command line here   */
{
   ...
   if(argc-my_optind ==8)
   {
	...
	pcb.HH=6*pcb.hh+pcb.tt;
      check_parameters_of_generate_bmp_for_coupled_microstrip(pcb);
      convert_generate_coupled_microstrip_bitmap_dimensions_to_integers(&pcb);
      W=pcb.W;
      H=pcb.H;
      ...
      usage_generate_bmp_for_coupled_microstrip();
   exit(0);
}
